...
class LightServer {
    ...
    boolean build(Options options) {
        ...
        assert(shadingCache==null|| shadingCache instanceof org.sunflow.core.LightServer.CacheEntry[]);
        ...
        assert(shaderOverride ==null || shaderOverride instanceof org.sunflow.core.Shader);
    ...
    boolean calculatePhotons(final PhotonStore map, String type, final int seed) {
        ...
        assert(lights instanceof org.sunflow.core.LightSource[]);
    ...
    void shadeBakeResult(ShadingState state) {
        Shader shader = getShader(state);
        if (shader != null)
            // Receiver Object shader needs four object sensitivity to precely resolve its target. 
            state.setResult(shader.getRadiance(state));
        else
            state.setResult(Color.BLACK);
    }
